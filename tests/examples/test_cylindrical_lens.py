from ezray.examples.cylindrical_lens import system, PARAXIAL_PROPERTIES
...
def test_back_principal_plane(pmid):
    if np.isnan(system.paraxial_models[pmid].back_principal_plane):
        assert np.isnan(PARAXIAL_PROPERTIES[pmid]["back_principal_plane"])
    else:
        assert np.allclose(
...
def test_front_principal_plane(pmid):
    if np.isnan(system.paraxial_models[pmid].back_principal_plane):
        assert np.isnan(PARAXIAL_PROPERTIES[pmid]["front_principal_plane"])
    else:
        assert np.allclose(
...
def test_paraxial_image_plane(pmid):
    ...
    if np.isnan(system.paraxial_models[pmid].paraxial_image_plane["semi_diameter"]):
        assert np.isnan(
